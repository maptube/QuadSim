...
public class QuadcopterBehaviourScript : MonoBehaviour {
	...
	float MaxThrust = 0; //you could just set this rather than messing about with the motor
	...
	//float txRollAngle, txPitchAngle, txYawRate; //desired roll, pitch and yaw angles or rates derived fron the transmitter stick positions and max angles
	//float bodyRollAngle, bodyPitchAngle, bodyYawRate; //gyro sensor values, measured from the aircraft matrix, used as PID inputs
...
//			throttle = pidAltitude.process(rb.position.y,AltitudeHold,Time.deltaTime);
		...
		//float thrust = 10.0f+throttle*6.0f; //t*5.0
		//float thrust = 4.8f+throttle*6.0f;
		//ailerons and elevator set requested angles to horizontal, rudder sets rotation speed
		//Convert transmitter control inputs to desired roll, pitch, yaw angles or rates
		//txRollAngle = -aileron * MaxRollAngle; //degrees - TODO: need to fix axis problem here
		//txPitchAngle = -elevator * MaxPitchAngle; //degrees (40)
		//txYawRate = rudder * MaxYawRate; //degrees per second (?) (24)

		//Simulate gyro sensor on the aircraft by measuring current body frame angles
		Vector3 euler = rb.transform.localEulerAngles; //was eulerAngles, but localEulerAngles seems better?
		//roll angle (degrees)
		...
		//Vector2 relAV = rb.angularVelocity * rb.transform.forward;
		Vector3 relAV = transform.InverseTransformDirection(rb.angularVelocity);
		//gyroRates.x = -rb.angularVelocity.z;
		gyroRates.x = -relAV.z;
		//pitch rate
		//gyroRates.y = -rb.angularVelocity.x;
		gyroRates.y = -relAV.x;
		//yaw rate
		//gyroRates.z = rb.angularVelocity.y;
		gyroRates.z = relAV.y;

		//convert angles to radians - why aren't they radians already?
		//bodyRollAngle = bodyRollAngle * Mathf.PI / 180.0f;
		//bodyPitchAngle = bodyRollAngle * Mathf.PI / 180.0f;
		//bodyRollRate = bodyRollRate * Mathf.PI / 180.0f;
		//bodyPitchRate = bodyPitchRate * Mathf.PI / 180.0f;
		//bodyYawRate = bodyYawRate * Mathf.PI / 180.0f;
		...
		//TODO: check how the Cleanflight P8[PIDLEVEL] constant.
		...
		//these are the working ones direct from PID
		rb.AddRelativeForce (new Vector3 (0, throttle*MaxThrust*1+4.8f, 0));
